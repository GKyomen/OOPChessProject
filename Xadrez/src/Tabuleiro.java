public class Tabuleiro {
    ...
    private boolean verificaXequeMate(String corAtacante) {
        ...
        Posicao posAtacante[] = posicaoAtacante(pRei, corAtacante); //só vai ter um obrigatoriamente
        if(estaAtacada(posAtacante[0], corAtacado)) {
        ...
        if(!(posAtacante[0].getPeca() instanceof Cavalo)) { //só dá se não for um cavalo
    ...
    private boolean xequeDescoberto(Posicao posRei, Posicao objetivo, String corPeca) {
        ...
        for (Posicao pos : atacaObjetivo) {
            if(!estaAtacadaNaDirecao(pos, corAtacante, qualCaminho(posRei, pos))) {
                return false;
            }
        }
        return true;
